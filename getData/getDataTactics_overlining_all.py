...
def data_write(res, date):
    ...
    res.to_sql(name="tactics_tactics_overlining_all_1", con=engine, if_exists="replace", index=False)
...
#该标识
query_sql = "SELECT trade_date FROM trading_day_date WHERE is_tactics_overlining_all = 0 AND trade_date BETWEEN '2020-01-01' AND '2023-12-31'  ORDER BY trade_date"
get_data_dates = arc.select_data(query_sql)
# print(get_data_date)
for get_data_date in get_data_dates:
    # 记录开始时间
    start_time = time.time()
    var = (get_data_date[0].strftime("%Y-%m-%d")+ tactics_overlining_all)
